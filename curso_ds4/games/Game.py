...
class Game:
    ...
    def play(self):
        ...
        a = choice(points)
        b = choice(points)
        # print(f"League: {league}|{points}")
        self.score[self.A.name] = a
        self.score[self.B.name] = b
    
    def __str__(self):
        '''Metodo para representar la clase como string'''
        return f"Game: {self.A.name} : {self.score[self.A.name]} vs {self.B.name} : {self.score[self.B.name]}"
...
if __name__ == "__main__":
    ...
    team_b = Team("Crazy Team", basketball, players_b)        
    ...
    print (f"Archivo {filename_json} creado")
